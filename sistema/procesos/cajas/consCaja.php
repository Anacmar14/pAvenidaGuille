...
switch($opcion){//Consulta por estado de caja
    case 1:   
        ...
        $numerito = count($data);
        if ($numerito == 0) {
            $consulta = "INSERT INTO cajas (cjfecha, cjmontoincial, cjcierre, cjsaldo, cjtoting, cjtotegr, cjfechahoracierre, empid) VALUES ('$date' ,0 , 0 , 0 , 0 , 0 , 0 , 1);";
            $resultado = $conexion->prepare($consulta);
            $resultado->execute();
    ...
    case 4: 
        $consulta = "SELECT movid, movimientos.movtipo, movnombre, movdinero, movdesc FROM `movimientos` INNER JOIN cajas ON empid = 1 AND cjfecha = CURRENT_DATE INNER JOIN tipomovimiento ON movimientos.movtipo = tipomovimiento.movtipo";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();        
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
